...
/**
 * Autocompleta lugares o direcciones con prioridad según la proximidad del usuario (búsqueda abierta en todo el país)
 */
export async function autocompletePlaces(text, proximity = SAN_ANTONIO_PROXIMITY, limit = 20) {
  if (!GEOAPIFY_API_KEY || !text || text.trim().length < 2) return [];
  try {
    const url = `https://api.geoapify.com/v1/geocode/autocomplete?text=${encodeURIComponent(text.trim())}&bias=proximity:${proximity.lon},${proximity.lat}&filter=countrycode:gt&limit=${limit}&apiKey=${GEOAPIFY_API_KEY}&lang=es`;
    const res = await fetch(url);
    if (!res.ok) return [];
    const data = await res.json();
