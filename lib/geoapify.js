...
function areRoutesDifferent(r1, r2) {
  ...
  // Si la distancia difiere en al menos 40 metros
  if (Math.abs(r1.distanceMeters - r2.distanceMeters) >= 40) return true;
  // Si el tiempo difiere en al menos 30 segundos
  if (Math.abs(r1.timeSeconds - r2.timeSeconds) >= 30) return true;
  // O si la geometría en el punto medio se separa más de ~35 metros
  const c1 = r1.coordinates || [];
  const c2 = r2.coordinates || [];
  if (Math.abs(c1.length - c2.length) > 5) return true;
  const mid1 = c1[Math.floor(c1.length / 2)];
  const mid2 = c2[Math.floor(c2.length / 2)];
  if (!mid1 || !mid2) return false;
  const dLat = Math.abs(mid1[0] - mid2[0]);
  const dLng = Math.abs(mid1[1] - mid2[1]);
 ...
 */
export async function calculateRoutes(start, end, mode = 'walk') {
 ...
 */
export async function calculateRoute(start, end, mode = 'walk') {
