...
/**
 * Autocompleta lugares o direcciones con prioridad en el entorno cercano (radio de 25 km)
 */
export async function autocompletePlaces(text, proximity = SAN_ANTONIO_PROXIMITY, radiusMeters = 25000) {
  if (!GEOAPIFY_API_KEY || !text || text.trim().length < 2) return [];
  try {
    const filterParam = radiusMeters
      ? `filter=circle:${proximity.lon},${proximity.lat},${radiusMeters}&filter=countrycode:gt`
      : 'filter=countrycode:gt';
    const url = `https://api.geoapify.com/v1/geocode/autocomplete?text=${encodeURIComponent(text.trim())}&bias=proximity:${proximity.lon},${proximity.lat}&${filterParam}&apiKey=${GEOAPIFY_API_KEY}&lang=es`;
    const res = await fetch(url);
    if (!res.ok) return [];
    const data = await res.json();
