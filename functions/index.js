...
async function generateSitemapContent(lastmod) {
  try {
    ...
    sitesSnapshot.forEach((doc) => {
      ...
      // LÓGICA COHERENTE CON FRONTEND (SiteCard.jsx)
      if (data.category && data.slug) {
        // Opción A: URL limpia con slug
        const catSlug = slugify(data.category, {
          lower: true, strict: true, locale: "es",
        });
        const siteSlug = slugify(data.slug, {
          lower: true, strict: true, locale: "es",
        });

        urls.push({
          loc: `${DOMAIN}/categoria/${catSlug}/${siteSlug}`,
          priority: "0.8",
          changefreq: "weekly",
          lastmod: lm,
        });
      } else {
        // Opción B: Fallback a ID si no hay slug
        urls.push({
          loc: `${DOMAIN}/sitio/${doc.id}`,
          priority: "0.8",
          changefreq: "weekly",
          lastmod: lm,
        });
 ...
 */
exports.updateSitemapScheduled = onSchedule("every 24 hours", async (event) => {
  try {
    const sitemapXml = await generateSitemapContent(null);
