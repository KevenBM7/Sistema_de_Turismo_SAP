...
function SiteDetailPage() {
  ...
  const { currentUser, toggleFavorite, loading: authLoading } = useAuth(); // Usamos el estado de carga del contexto
  // Estados para el modal de la imagen
  const [isImageModalOpen, setIsImageModalOpen] = useState(false); // Mantenemos este para saber si está abierto
  const [selectedImageIndex, setSelectedImageIndex] = useState(null); // Usamos el índice para navegar
  const navigate = useNavigate();

  useEffect(() => {
    // --- SOLUCIÓN: Scroll al inicio de la página al montar el componente ---
    window.scrollTo(0, 0);
    ...
    const fetchSite = async () => {
      ...
      try {
        ...
        if (slug) {
          ...
          // Búsqueda por ID (URL de respaldo)
        ...
        if (!siteDoc) {
          ...
          if (siteData.imagePaths && Array.isArray(siteData.imagePaths)) {
            const urlPromises = siteData.imagePaths.map(async (pathData) => {
              ...
              // Generar URLs para cada tamaño en formato WebP
              const sizes = [150, 800, 1200]; // Usar los tamaños que sí existen: 150, 800, 1200
              const srcset = {};
              for (const size of sizes) {
                // Las imágenes redimensionadas ESTÁN en la misma carpeta "originals/".
                const sizedPath = imagePath.replace(/(\.[\w\d_-]+)$/i, `_${size}x${size}.webp`);
                try {
                  srcset[`${size}w`] = await getDownloadURL(ref(storage, sizedPath));
                } catch (e) { /* Ignorar si una versión no existe, ya que la original se elimina */ }
              }
              
              // SOLUCIÓN DEFINITIVA: Usar una de las versiones existentes como la principal, sin recurrir a la original.
  ...
  if (loading) return <SiteDetailSkeleton />; // Usar el skeleton mientras carga
  ...
  const handleShare = () => {
    // Extrae el texto plano de la descripción HTML
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = site.description || site.description_es || '';
    const plainTextDescription = tempDiv.textContent || tempDiv.innerText || "";
    const shortDescription = plainTextDescription.substring(0, 80) + (plainTextDescription.length > 80 ? '...' : '');

    // Texto que se compartirá
    const shareText = `¡Echa un vistazo a este lugar en San Antonio Palopó!\n\n*${site.name}*\n${shortDescription}`;
    const shareUrl = window.location.href;

    // Detectar si es un dispositivo móvil para ofrecer compartir en WhatsApp
    const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);

    if (isMobile && navigator.share) {
      // Usar la API de compartir nativa en móviles
      navigator.share({
        title: site.name,
        text: `${shareText}\n\nDescubre más aquí:`,
        url: shareUrl,
  ...
  // El estado de favorito ahora se deriva directamente del contexto
  ...
  const sliderSettings = {
    ...
    pauseOnHover: true,     // Se detiene si el usuario pasa el mouse por encima
  ...
  const modalSliderSettings = {
    ...
    arrows: true,
    initialSlide: selectedImageIndex, // Iniciar en la imagen seleccionada
  ...
  return (
    <div className="site-detail-container">
      {/* Contenedor para Categoría y Botón de Editar */}
      ...
      {/* Carrusel de Imágenes con contenedor para las flechas */}
            ...
            {site.imageUrls.map((url, index) => (
              <div key={index} className="carousel-slide-wrapper">
                <img 
                  ...
                  loading={index === 0 ? 'eager' : 'lazy'} // Carga la primera imagen de inmediato
                  fetchPriority={index === 0 ? 'high' : 'auto'} // Prioriza la primera imagen
                  width="800" height="450" // Dimensiones explícitas para evitar CLS
                />
              </div>
            ))}
          </Slider>
      ...
      <div className="site-actions-container">
        ...
        {(site.whatsapp || site.facebook || site.instagram) && (
