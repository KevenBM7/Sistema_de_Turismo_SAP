...
import { collection, query, onSnapshot, orderBy, doc, deleteDoc, collectionGroup, where, getDoc } from 'firebase/firestore';
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (siteIdParam) {
      const fetchSite = async () => {
        const docRef = doc(db, 'sites', siteIdParam);
    ...
    if (eventIdParam) {
      const fetchEvent = async () => {
        const docRef = doc(db, 'events', eventIdParam);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          setEventToEdit({ id: docSnap.id, ...docSnap.data() });
  ...
  const handleBackToMenu = () => {
    ...
    setSelectedSubcategory('');
    // Limpiar query params para evitar re-carga al refrescar
    router.push('/admin');
  };
  ...
  const handleDeleteSite = (siteId, imagePaths) => {
    const performDelete = async () => {
      ...
      // 3. Revalidar (ya no existe el sitio, pero la categoría y el home deben actualizarse)
      await triggerRevalidation('site', { slug: 'removed', categoryName: 'general' }); 
      router.refresh();
    };
  ...
  const handleDeleteEvent = (eventId, imagePath) => {
    const performDelete = async () => {
      ...
      await triggerRevalidation('event', { slug: 'removed' });
      router.refresh();
    };
