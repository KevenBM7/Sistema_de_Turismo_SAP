...
function MapPage() {
  ...
  const mapZoom = initialSelectedSite ? 14 : 13;
  ...
  const handleRoutesFound = React.useCallback((foundRoutes) => {
    // Evitar bucle infinito: solo actualizar si hay cambios reales en los datos de la ruta
    setRoutes(prev => {
      if (prev.length === foundRoutes.length && 
          prev[0]?.summary?.totalDistance === foundRoutes[0]?.summary?.totalDistance) {
        return prev;
      }
      return foundRoutes;
    });
      ...
      >
        ...
        {sites.map(site => (
          ...
          >
            ...
            <Link href={`/categoria/${site.categorySlug || slugify(site.parentCategory || 'general', { lower: true, strict: true })}/${site.slug}`} className="popup-link">Ver detalles</Link>
          </div>
        </Popup>
      </Marker>
    ))}
