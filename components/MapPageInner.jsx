...
function MapPage() {
  ...
  // 3. Búsqueda y autocompletado inteligente (Sitios Firebase + Lugares Geoapify en todo el mapa)
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      // Sitios turísticos registrados en Firebase con distancia calculada
      ...
      // Lugares, calles y comercios en general (priorizando cercanía al usuario o región)
      let geoMatches = [];
      try {
        const proximityCoords = (isRealLocationAvailable && userLocation)
          ? { lat: userLocation.lat, lon: userLocation.lng }
          : { lat: SAN_ANTONIO_PALOPO[0], lon: SAN_ANTONIO_PALOPO[1] };

        const places = await autocompletePlaces(searchQuery, proximityCoords);
      ...
      let combined = [...localMatches, ...geoMatches];
    ...
    <div className="map-page-container">
      ...
      {!isSearchOpen ? (
        ...
        <div className="map-search-container expanded" ref={searchContainerRef}>
          ...
          {/* Desplegable de Resultados de Búsqueda con Distancia en Vivo */}
